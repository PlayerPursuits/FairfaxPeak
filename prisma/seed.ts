...
async function main() {
  ...
  await db.user.create({
    data: { email: "admin@fairfaxpeak.example", name: "Site Admin", passwordHash, role: "ADMIN" },
  });
  const resident = await db.user.create({
    data: { email: "resident@example.com", name: "Riley Resident", passwordHash, role: "PERSONAL", newsletter: "WEEKLY", ...town },
  });
  const reviewers = [];
  for (const [name, email] of REVIEWERS) {
    reviewers.push(await db.user.create({ data: { email, name, passwordHash, role: "PERSONAL", newsletter: "DAILY" } }));
  }

  for (const [bi, b] of BUSINESSES.entries()) {
    const periodEnd = days(30);
  ...
  const memberOffer = await db.offer.findFirst({ where: { exclusive: true } });
