...
export function BusinessCard({ b }: { b: Business }) {
  ...
  return (
    <Link href={`/business/${b.slug}`} className="card group flex h-full flex-col overflow-hidden text-inherit no-underline transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="relative h-36 bg-gradient-to-br from-pine-100 to-pine-200">
        {cover && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={cover} alt="" className="h-full w-full object-cover" />
        )}
        {b.offerCount ? (
          <span className="badge absolute top-3 right-3 bg-sun-400 text-pine-900 shadow">
            {b.offerCount} offer{b.offerCount > 1 ? "s" : ""}
          </span>
        ) : null}
      </div>
      <div className="flex flex-1 gap-3 p-4">
        <div className="-mt-10 flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl border-4 border-white bg-pine-700 text-xl text-white shadow">
          {b.logoUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
        ...
        <div className="min-w-0">
          <h3 className="truncate font-semibold group-hover:text-pine-700">{b.name}</h3>
